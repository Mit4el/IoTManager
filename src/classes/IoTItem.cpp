...
IoTItem::IoTItem(const String &parameters)
{
    ...
    if (!jsonRead(parameters, F("multiply"), _multiply, false))
        _multiply = 1;
    if (!jsonRead(parameters, F("plus"), _plus, false))
        _plus = 0;
    if (!jsonRead(parameters, F("round"), _round, false))
        _round = -1;
    if (!jsonRead(parameters, F("numDigits"), _numDigits, false))
        _numDigits = 1;

    if (!jsonRead(parameters, F("global"), _global, false))
        _global = false;
...
void IoTItem::suspendNextDoByInt(unsigned long _delay)
{ // 0 - force
...
float IoTItem::getValue()
{
    ...
    // округляем приводим к float и выдаём
    if (_round >= 0 && _round <= 6)
    {
        int sot = _round ? pow(10, (int)_round) : 1;
        *ptr = round(*ptr * sot) / sot;
    }
    return (float)*ptr;
}
...
void IoTItem::setValue(const float &valtmp, bool genEvent)
{
    ...
    {
        int sot = _round ? pow(10, (int)_round) : 1;
        valtmp = round(valtmp * sot) / sot;
    }
    if (_multiply)
        valtmp = valtmp * _multiply;
    if (_plus)
        valtmp = valtmp + _plus;
    if (_map1 != _map2)
...
void IoTItem::regEvent(const IoTValue &val, bool error, bool genEvent)
{
    if (_needSave)
    {
...
{
    if (interval == 0)
        enableDoByInt = false; // выключаем использование периодического выполнения в модуле
    else
    {
        enableDoByInt = true;
        if (interval > 0)
            _interval = interval * 1000; // если int положителен, то считаем, что получены секунды
        else if (interval < 0)
            _interval = interval * -1; // если int отрицательный, то миллисекунды
    }
...
{
    if (!jsonRead(msg, F("id"), altId, altId == ""))
        return; // ничего не предпринимаем, если ошибка и altId = "", вообще данная конструкция нужна для совместимости с форматом данных 3 версией
    ...
    if (itemExist)
    {
        ...
        if (interval)
            itemExist->setIntFromNet(interval + 5); // если пришедший интервал =0, значит не нужно контролировать доверие, иначе даем фору в 5 сек
